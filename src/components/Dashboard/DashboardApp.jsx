...
export default function DashboardApp({ 
  ...
  useEffect(() => {
              ...
              setPastShorts(prev => prev.map(t => 
                (t.threadId === activeThreadId || t.id === activeThreadId)
                  ? { 
                      ...
                      messages: [
                        ...(t.messages || []),
                        { role: 'assistant', content: `🎬 Final 5 scenes generated by Split Scenes: "${data.story?.title || t.title}". Review before video rendering.` }
                      ]
                    }
                  : t
              ));
              ...
              setPastShorts(prev => prev.map(t => 
                (t.threadId === activeThreadId || t.id === activeThreadId)
                  ? { 
                      ...
                      messages: [
                        ...(t.messages || []),
                        { role: 'assistant', content: `Story ready for review: "${data.story?.suggestedTitle || data.title}"` }
                      ]
                    }
                  : t
              ));
    ...
    if (isGenerating) {
      ...
      pollInterval = setInterval(checkNetlifyStoryApproval, 1500);
